/**
 * SGTI Edge Functions — SLA calculation utilities (Docs/31_SLA.md §2, §7, §8).
 *
 * Mirrors lib/utils/sla.ts (Next.js app) — duplicated rather than shared
 * because Edge Functions (Deno) and the Next app (Node) are separate
 * deployable units with no shared package between them in this repo.
 *
